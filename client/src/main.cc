...
main (int argc, char* argv[]) {
    ...
    hidden.add_options()
        ("raddr,s", po::value<std::string>(&raddr), "Remote address")
        ("rport,r", po::value<std::string>(&rport)->implicit_value("8080"), "Remote port");
    ...
    BOOST_LOG_TRIVIAL(info) << "Using remote server " << raddr << ":" << rport;
